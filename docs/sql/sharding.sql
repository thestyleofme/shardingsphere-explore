...
(
    ...
    `status`          varchar(50) NOT NULL COMMENT '投递状态 投递状态 WAIT-待处理 AUTO_FILTER-自动过滤 PREPARE_CONTACT-待沟通 REFUSE-拒绝 ARRANGE_INTERVIEW-通知面试',
    `create_time`     datetime    NOT NULL COMMENT '创建时间',
    `operate_time`    datetime    NOT NULL COMMENT '操作时间',
    `work_year`       varchar(50)          DEFAULT NULL COMMENT '工作年限',
    `name`            varchar(50)          DEFAULT NULL COMMENT '投递简历人名字',
    ...
    KEY `index_companyId_status` (`company_id`, `status`(50), `is_del`),
...
(
    `Id`         bigint(11) NOT NULL AUTO_INCREMENT,
    `name`       varchar(50) DEFAULT NULL,
    `pwd_plain`  varchar(50) DEFAULT NULL,
    `pwd_cipher` varchar(50) DEFAULT NULL,
    PRIMARY KEY (`Id`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4;